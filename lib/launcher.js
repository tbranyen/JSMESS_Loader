...
define(function(require) {
  ...
  var init = params.game ? ready : getgamelist;
  init(moduleName);

  function getextrahtml(name, folder, file) {
    // If there is an element allocated for the machine's information, fetch it
    // asynchronously.
    if (moduleinfo) {
      moduleinfo.addEventListener('load', function() {
        var innerHeight = this.contentWindow.document.body.offsetHeight + 'px';

        // The inner height of the iframe.
        moduleinfo.style.height = innerHeight;
        moduleinfo.setAttribute('class', 'visible');
      }, true);

      // Display the HTML content.
      moduleinfo.src = folder + '/' + file + '.html';
    }
  }
